...
namespace LDAPClient
{
    ...
    {
        public LDAPApi(string username, string domain, string password, string url)
        {
        ...
        public Dictionary<string, string> getUserAttributes(string username, List<string> attribues)
        {
            Dictionary<string, string> result = new Dictionary<string, string>();
            //TODO: refactor to public property on class
            string baseNode = "CN=Users,DC=dq,DC=com,DC=lb";

            var searchResult = search(baseNode, "sAMAccountName=" + username);

            foreach (string requestedAttr in attribues)
            {
                result.Add(requestedAttr, searchResult[0][requestedAttr]);
            }
